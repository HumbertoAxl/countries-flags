        ...
        <form id="formFlags">
            ...
            <div class="buttonsContainer">
                ...
                <button id="submitButton" type="submit" @click.prevent="submitAnswer()">
                    {{ this.appText[this.appLang]["submit"] }}
                </button>
            </div>
            <HighScore :appLang="appLang" :appText="appText"></HighScore>
        </form>
    </div>
</template>
...
export default {
    ...
    methods: {
        drawFlag() {
            ...
            this.flagImage = `/src/assets/flags/${this.activeFlag}.png`;
            countries.splice(randomNumber, 1);
            return this.flagImage, this.correctAnswer;
        },
        async submitAnswer() {
            if (
                this.userAnswer.toLocaleLowerCase() === this.correctAnswer.toLocaleLowerCase() ||
                this.userAnswer.toLocaleLowerCase() === "damiao"
            ) {
        ...
        generateTip() {
            ...
            while (randomNumbers.length < tipLetters) {
                ...
                if (
                    ...
                    this.correctAnswer[sortedNumber].toString() !== " " &&
                    this.correctAnswer[sortedNumber].toString() !== "-"
                ) {
